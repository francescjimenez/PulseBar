export default function BarChart({ primary, secondary, colorP, colorS, height = 52, label = "Usage history chart" }) {
  const N = primary.length;
  if (N === 0) return null;
  const max = Math.max(...primary, ...(secondary || []), 0.001);
  const W = N * 4;
  return (
    <svg className="bar-chart" width="100%" height={height} viewBox={`0 0 ${W} ${height}`} preserveAspectRatio="none" role="img" aria-label={label}>
      <title>{label}</title>
